...
Port `3838` is used in the example below, but any port between 3000 and 8000 can be used, except for ports [blocked by browsers](https://fetch.spec.whatwg.org/#port-blocking) due to security concerns. *Invalid ports* include 3659, 4045, 5060, 5061, 6000, 6566, 6665-6669, 6679, and 6697. 
...
# Step 4: Set Up the Shiny Application {#step-4}

The app.R script is the entry point for the Shiny app. This should involve a call to `run_scExploreR` 
...
# Step 5: Generate App Config File {#step-5}
...
If you use a different port than `3838`, replace `3838` with your port number. For example, to run on port `3850`, use:

```bash
docker run --rm -p 3850:3850 scexplorer
```

# Accessing the Application

Once the container is running, open your web browser and navigate to `http://localhost:3838` (If you use a different port, replace `3838` with your port number). You should see the scExploreR interface.


# Troubleshooting
## Common Issues

- Cannot Access App: Ensure the container is running and the port chosen (`3838` in this tutorial) is not blocked by a firewall. Also ensure that the port chosen is not on the [list of ports considered unsafe](https://fetch.spec.whatwg.org/#port-blocking) and therefore blocked by browsers.
...
- [List of Unsafe Ports](https://fetch.spec.whatwg.org/#port-blocking)