...
Each categorical metadata variable can be used to filter the dataset. This selection is controlled for each variable independently, including with the use of the "select all" and "remove filter" button. Criteria defined by each menu are combined using `AND` syntax, meaning that only cells simultaneously meeting all filter criteria will be returned upon subsetting. Menus for the other variables will update based on possible combinations made in the current menu, and invalid combinations will appear in gray. Press "Apply Subset" to create the subset from the current selection.
