...
<style type="text/css">
  ...
  .callout-default h4, .callout-default h5 {
    color: #777;
  }
  .callout-primary {
    border-left-color: #428bca;
    background-color: #428bca35;
  }
  .callout-primary h4, .callout-primary h5 {
    color: #428bca;
  }
  .callout-success {
    border-left-color: #5cb85c;
    background-color: #5cb85c35;
  }
  .callout-success h4, .callout-success h5 {
    color: #5cb85c;
  }
  .callout-danger {
    border-left-color: #d9534f;
    background-color: #d9534f35;
  }
  .callout-danger h4, .callout-danger h5 {
    color: #d9534f;
  }
  .callout-warning {
    border-left-color: #f0ad4e;
    background-color: #f0ad4e35;
  }
  .callout-warning h4, .callout-warning h5 {
    color: #f0ad4e;
  }
  .callout-info {
    border-left-color: #5bc0de;
    background-color: #5bc0de35;
  }
  ...
  .callout-bdc {
    ...
    background-color: #29527a35;
  }
  .callout-bdc h4, .callout-bdc h5 {
    color: #29527a;
  }
    </style>
```
...
### Metadata for Labeling Groups {#feature-plots-labeling-groups}

This menu appears when [Label Groups](#feature-plots-label-groups) is checked. It selects the categorical metadata variable used to label groups of cells on the feature plot. Labels will be placed above each cluster of cells as defined by the selected variable.
...
Toggles group by variable labels above each set of cells on and off. When checked, the [Metadata for Labeling Groups](#feature-plots-labeling-groups) menu will appear to select the metadata variable used for labels.
...
Violin plots show the distribution of feature expression and can be used to compare expression distributions between groups of cells.
...
For defined groups of cells, dot plots show the average expression per cell group, as well as the percentage of cells with non-zero expression of the feature. Dot plots are especially useful for large numbers of features.
...
DGE is not available for SingleCellExperiment objects because there is no known package that can process both the standard sparse matrices and the HDF5-enabled DelayedArray matrices in these objects. See the [object guidelines](object_guidelines.html) article for more details.
:::
...
The Average Expression column is only returned for Seurat and BPCells-backed Seurat objects. It is not present in DGE results for anndata objects. See the [object guidelines](object_guidelines.html) article for more details on feature availability by object class.
:::
...
The AUC column is only returned for standard Seurat objects. It is not present in DGE results for BPCells-backed Seurat objects, anndata objects, or SingleCellExperiment objects.
:::

-   **Raw p-value:** the unadjusted p-value from the statistical test comparing expression of the gene in the current group vs. the other group, or all other cells.
...
The Percent Expression columns are only returned for standard Seurat and anndata objects. They are not present in DGE results for BPCells-backed Seurat objects.
:::
...
To change the dataset, press the `r icon("ellipsis-h")` button in the upper right hand corner of the app window, and select **"Choose Dataset"**. A window will appear with the datasets that have been added to the browser, with a description of each. Select a dataset and press "confirm selection" to load the dataset. The dataset can be changed from any tab, and the loaded dataset will affect all three tabs.
