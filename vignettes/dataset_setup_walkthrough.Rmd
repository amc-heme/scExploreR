...
::: {.card .mb-3}
::: {.card-header}
<h4 class = "card-title center"> Guiding Questions for Metadata Selection </h4>
:::
...
`datasets` is a list of all the datasets in the app. Each key under `datasets` represents a dataset to be loaded in the app instance (in this case, the dataset keys are `combined_sclc` and `epithelial_sclc`). 

**Important**: Dataset names (the keys under `datasets`) **must not contain spaces**. Use underscores or other characters instead (e.g., `my_dataset` instead of `my dataset`). Dataset names are used internally as Shiny module identifiers, and spaces will cause the app to fail with an error. The app will validate dataset names on startup and provide a clear error message if spaces are detected.
