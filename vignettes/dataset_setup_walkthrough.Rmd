...
when making edits to this Rmd, please do not add any evaluating R chunks. This
vignette was designed to have markdown code only, rather than live R code, 
since it needs to be knit in a GitHub Actions Runner to post to the pkgdown
website.

If you add an R code chunk, plase add eval=FALSE to the end. 

In several locations in this document, markdown-only R chunks exist. These have 
r after triple backticks without braces:

``` r instead of ``` {r}.

If you add braces to execute these chunks locally, please also add eval=FALSE to
ensure they don't execute on the GitHub runner.
...
-->
```
-   Combined samples <!-- -   Epithelial cells -->
-   SCLC epithelial cells

Chan et al. use these datasets in their analysis, and the same will be done in this example. The "Epithelial cells" dataset is a subset of the "Combined samples" dataset that has been re-clustered. Likewise, the "SCLC epithelial cells" dataset is a subset of the Epithelial cells dataset. See **Figure 1A** of Chan et al. for a visual representation.
...
full_dataset <- 
  anndata::read_h5ad(
    ...
    "~/scExplorer/datasets/Epithelial_SCLC.h5ad"
    )
```
...
```{=html}
<!-- My output for this was a bit different, FYI
 [1] "n_genes_by_counts"                        "log1p_n_genes_by_counts"                  "total_counts"                            
 [4] "log1p_total_counts"                       "mito_frac"                                "RBP_frac"                                
 [7] "batch"                                    "donor_id"                                 "treatment"                               
[10] "procedure"                                "SCLC_subtype"                             "SCLC_subtype_plus_TP53_RB1_wt"           
[13] "pval_SCLC-A"                              "pval_SCLC-N"                              "pval_SCLC-P"                             
[16] "NEUROD genes"                             "MHCI_PRESENTATION"                        "MHCII_PRESENTATION"                      
[19] "SCLC_major_subtype_of_sample"             "clusters"                                 "treatment_categorized"                   
[22] "recurrent_cluster"                        "histo"                                    "clusters_fine"                           
[25] "disease_ontology_term_id"                 "tissue_ontology_term_id"                  "cell_type_ontology_term_id"              
[28] "organism_ontology_term_id"                "is_primary_data"                          "assay_ontology_term_id"                  
[31] "sex_ontology_term_id"                     "self_reported_ethnicity_ontology_term_id" "development_stage_ontology_term_id"      
[34] "suspension_type"                          "HTAN_Biospecimen_ID"                      "HTAN_Participant_ID"                     
[37] "tissue_type"                              "cell_type"                                "assay"                                   
[40] "disease"                                  "organism"                                 "sex"                                     
[43] "tissue"                                   "self_reported_ethnicity"                  "development_stage"                       
...
```{=html}
<!-- My output was different, as above:
                        observation_joinid                                    batch                                 donor_id                                treatment 
                             "character"                                 "factor"                                 "factor"                                 "factor" 
                               procedure                                    histo                         cell_type_coarse                           cell_type_fine 
                                "factor"                                 "factor"                                 "factor"                                 "factor" 
                       cell_type_general                                 clusters                            cell_type_med       development_stage_ontology_term_id 
                                "factor"                                 "factor"                                 "factor"                                 "factor" 
                    sex_ontology_term_id self_reported_ethnicity_ontology_term_id                   assay_ontology_term_id                  tissue_ontology_term_id 
                                "factor"                                 "factor"                                 "factor"                                 "factor" 
                disease_ontology_term_id                organism_ontology_term_id               cell_type_ontology_term_id                          suspension_type 
                                "factor"                                 "factor"                                 "factor"                                 "factor" 
                     HTAN_Biospecimen_ID                      HTAN_Participant_ID                              tissue_type                                cell_type 
                                "factor"                                 "factor"                                 "factor"                                 "factor" 
                                   assay                                  disease                                 organism                                      sex 
                                "factor"                                 "factor"                                 "factor"                                 "factor" 
                                  tissue                  self_reported_ethnicity                        development_stage                          is_primary_data 
                                "factor"                                 "factor"                                 "factor"                                "logical" 
                                  ngenes                                  libsize                                mito_frac                                 RBP_frac 
...
anndata::write_h5ad(
  full_dataset,
  filename = "~/scExploreR/datasets/SCLC_Full.h5ad"
)

anndata::write_h5ad(
  sclc_dataset,
  filename = "~/scExploreR/datasets/Epithelial_SCLC.h5ad"
)
```

## Launching the Config App
<!-- note about installing scExploreR with link to install guide -->
...
scExploreR::run_config(
  object_path = "~/scExploreR/datasets/SCLC_Full.h5ad"
  )
```

```{r, include=FALSE, eval=FALSE}
# Actual path to object for vignette construction
scExploreR::run_config(
  object_path = "~/1A_Jordan_Lab/Objects/small_cell_lung_cancer/SCLC_Full.h5ad"
  )
```
...
# Actual path to object for vignette construction
scExploreR::run_config(
  object_path = "~/1A_Jordan_Lab/Objects/small_cell_lung_cancer/SCLC_Epithelial_SCLC_lzf.h5ad"
  )
```
