...
# Laod libraries used for Rmd components
library(htmltools)
library(bslib)
library(shiny)
```

```{scss document css, echo = FALSE}
...
/* CSS for callouts
   ...
   /* Requires SASS to be enabled, with "scss" code chunk*/
.callout {
  padding: 20px;
  margin: 20px 0;
...
```{r load datasets-local, include=F}
library(anndata)

full_dataset <- anndata::read_h5ad("~/1A_Jordan_Lab/Objects/small_cell_lung_cancer/SCLC_Full.h5ad")

sclc_dataset <- anndata::read_h5ad("~/1A_Jordan_Lab/Objects/small_cell_lung_cancer/SCLC_Epithelial_SCLC_lzf.h5ad")
```
...
```{r combined categorical vs. numeric, class.output = "scroll-300"}
...
```{r full object plots, message = F, fig.width = 12, fig.height = 12}
library(patchwork)
...
```{r unique donor ids, class.output="scroll-100"}
SCUBA::unique_values(
  object = full_dataset,
  var = "donor_id"
) |> 
...
We see that `r scExploreR:::vector_to_text(names(var_classes[var_classes == "factor"]))` are categorical variables. Let's plot them below to better understand them. `clusters` and `treatment` are added for comparison with `clusters_fine` and `treatment_categorized`, respectively.

```{r sclc categorical plots, message = F, fig.width = 16, fig.height = 20}
cell_type_vars <- 
  c("SCLC_subtype",
...
```{python set up scExploreR_assays, eval = FALSE}
...
bslib::accordion(
  ...
  bslib::accordion_panel(
    # Add space before the image below the accordion
    style = "margin-bottom: 15px;",
    title = "Suggested description for this example",
    tags$p(
      "147k cells from 19 patients. Twenty-one small cell lung cancer samples are included, along with 24 lung
...
Now that we've filled out all the information in the config file, let's save the file to disk. To do this, navigate to the orange `r icon("ellipsis-h")` button, select **"Save Config File"**, and use the system prompt that appears to save the file. The file will save with a ".yaml" extension.
...
```{r sclc metadata labels, echo = FALSE, message = FALSE}
meta_vars <- 
...
```{r multi object app example, eval = FALSE}
scExploreR::run_scExploreR(
