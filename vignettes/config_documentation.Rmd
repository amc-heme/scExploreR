...
Both categorical and numeric metadata may be included in the object. Categorical variables can be added individually with per-variable settings (display name, description, etc.), while numeric variables are enabled or disabled all at once via the "Include numeric metadata" option. When enabled, numeric metadata will appear in the "enter a feature" menu and can be plotted like features on plots that involve feature expression, but cannot be used to group or split plots.
