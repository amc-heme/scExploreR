...
The `object_path` argument is required, and it must be an .rds containing a single Seurat object. The `config_path` argument is optional and is used to load an existing config file into the app for further editing (for more on this, see [loading a config file](#load-config-file) below).
