...
icon_notification_ui_2 <- function(icon_name,...){
  span(
    ...
    span(tagList(...),style="font-size: 1.17em;")
  )
}
###
...
github_link <- function(display_text,
...
#add_error_notification() will store information for an error message to be handled, 
#and the desired Shiny notification to show for that error. The argument
#notification_ui should be a shiny.tag or shiny.tag.list object, and it is 
#recommended to use the icon_notification_ui() function defined in this source file. 
add_error_notification <- function(message,notification_ui,notification_id){
  #Create a list to store information on the error, including the message and the
  #notification UI
  error <- list()
...
error_handler <- function(session,cnd_message,error_list,issue_href="https://github.com/amc-heme/DataExploreShiny/issues", id_prefix="",duration=NULL){
  ...
  for (error_type in error_list){
    ...
    if (grepl(cnd_message,error_type$err_message)){
      #Display Notification
      showNotification(ui=error_type$notification, 
