...
#metadata slot of the Seurat object, and the list object itself must be reactive, 
#as opposed to each individual item being reactive. The app code should generate the
#list in the correct format automatically.  
make_subset <- function(sobj, criteria_list){
  ...
  for (i in 1:length(criteria_list)){
    ...
    #cells with metadata matching all criteria are marked TRUE and included in 
    #the subset. The ampersand character is used to separate all criteria, to 
